...
class ConfigGUI:
    ...
    def create_window(self):
        ...
        # Handle window close - IMPORTANT: Hide instead of destroy
    ...
    def refresh_applications(self):
        ...
        try:
            ...
            common_apps = ['Spotify.exe', 'spotify.exe', 'chrome.exe', 'firefox.exe', 'discord.exe', 'steam.exe', 'vlc.exe']
    ...
    def on_closing(self):
        """Handle window closing - HIDE instead of destroy to keep app running"""
        try:
            self.window_open = False
            if self.root:
                self.root.withdraw()  # Hide instead of destroy
                logger.info("Configuration window hidden - HotVolume continues running in tray")
        except Exception as e:
